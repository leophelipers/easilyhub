...
const months=['Stycznia','Lutego','Marca','Kwietnia','Maja','Czerwca','Lipca','Sierpnia','Września','Października','Listopada','Grudnia',];(monthMin=['','','','','','','','','','','','']),(days=['duminică','luni','marți','miercuri','joi','vineri','sâmbătă']),(daysMin=['','','','','','','']),(seasons=['iarnă','arc','vară','toamnă'])
function postDate(daysName,daysMinName,monthsName,monthsMinName,seasonsName){const _counterLength=60
for(let counter=0;counter<_counterLength;counter++){innerDate(counter,'date-')
...
for(let i=0;i<nodeList.length;i++){const dateFormat=nodeList[i].dataset.format
...
function changeFormat(_day,_month,_year,format,counter){let innerFormat=format
const testFormat=['dd','mm','yyyy','monthFull','monthOnly','year'],dateFormat={dd:_day,mm:addZero(_month),yyyy:_year,monthFull:getMonthName(_month,monthsName,false),monthOnly:getMonthName(_month,monthsName,false,counter),year:getYearWithCounter(_year,counter),}
for(let i=0;i<testFormat.length;i++){let string=testFormat[i]
let regExp=new RegExp(string)
innerFormat=innerFormat.replace(regExp,dateFormat[string])}
return innerFormat.split(' ').join(' ')}
function getMonthName(_month,monthsName,bigFirstLetter,counter){const monthCounter=!!counter?counter:0
let month
