...
// export interface IAdminSideBarProps {} props: IAdminTopbarProps
...
/**
 * <Avatar className="shadow shadow-zinc-700">
        <AvatarImage src="https://github.com/leophelipers.png" />
      ...
      {MENU?.map((item) => (
        <div
          key={item.key}
          className="flex flex-row w-fit gap-1 items-center justify-center"
        >
          <Link
            href={item.url}
            className="flex flex-row gap-2 items-center justify-center py-2"
          >
            {item.icon} <h2 className="font-bold">{item.title}</h2>
          </Link>
        </div>
      ))}
 */
